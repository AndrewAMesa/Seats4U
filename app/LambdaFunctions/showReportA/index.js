...
exports.handler = async (event) => {
    ...
    if (validToken) 
    {
        let generateShowsReportVM = (token) => 
        {
            return new Promise((resolve, reject) => 
            {
                pool.query("SELECT Shows.*, Venues.location FROM Shows JOIN Venues ON Venues.venueName = Shows.venueName WHERE Venues.venueToken = ?", [token], (error, rows) => 
                {
        ...
        const allVenues = await generateShowsReportVM()
