...
exports.handler = async (event) => {
    ...
    if (validToken) {
        let DeleteVenue = (token) => {
            return new Promise((resolve, reject) => {
                  pool.query("DELETE FROM Venues WHERE venueToken =?", [token], (error, rows) => {
          ...
          const result = await DeleteVenue(event.venueToken)
        ...
        return response;
         ...
         const  response = {
            statusCode: 400,
            error:JSON.stringify("grinding and gnashing of teeth: wrong token")
          }
        pool.end()   // disconnect from database to avoid "too many connections" problem that can occur
        return response;
